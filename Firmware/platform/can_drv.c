...
u8 CAN1_Init(void)
{
    ...
    GPIO_Init(CAN1_GPIO, &GPIO_InitStructure);//初始化PA11,PA12
	
	//引脚复用映射配置
	GPIO_PinAFConfig(CAN1_GPIO,GPIO_PinSource8,GPIO_AF_CAN1); //GPIOB8复用为CAN1
	GPIO_PinAFConfig(CAN1_GPIO,GPIO_PinSource9,GPIO_AF_CAN1); //GPIOB9复用为CAN1
...
{	
	...
	while(len)
	{
		...
		i=0;
		while((CAN_TransmitStatus(CAN1, mbox)!= CANTXOK));	//等待发送结束
		len -= cur_len;
		msg += cur_len;	
	}
